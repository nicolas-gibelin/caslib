#     Copyright (C) 2017
#     Universite Grenoble Alpes: UMS Gricad
#     Updated from previous revision by Nicolas Gibelin
#     
#     Copyright (C) 2009
...
class CASServer(object):
    ...
    def __init__(self, baseurl, opener=None):
        ...
        self.urlopen = opener and opener.open or urllib.request.urlopen
...
def login_to_cas_service(url, username, password, service='', opener=None):
    ...
    :type password:        string or callable taking no arguments and returning a
                            string
    :type service:         string or callable taking no arguments and returning a
                            string
...
#        opener = urllib.request.build_opener(HTTPCookieProcessor(cj))
    ...
    try:
        ...
        raise CASLoginError('service could not be retrieved: %s', e)
    
    ##########################
    # Add service name to url
    ##########################
    # Extract query from url 
    params = {'service':service}
    url_parts = list(urlparse(url))
    query = dict(parse_qsl(url_parts[4]))
    # add 'service' param to query
    query.update(params)
    ...
    try:
        try:
            ...
            login_doc = ET.fromstring(login_str, parser=ET.HTMLParser(), base_url=url)

        except (AttributeError, SyntaxError) as lxml_error:
        ...
        if isinstance(lxml_error, SyntaxError):
            errors += ', %s' % lxml_error
        raise CASLoginError('Could not parse the document at %s: %s' % (url, errors))

    action, params = _get_login_bits(login_doc, url)

    if not action:
        raise CASLoginError('Could not find a CAS login form at %s' % url)
    ...
    data = urlencode(params, True).encode("utf-8")
    ...
    # But if using that library under proxy (search jupyterhub does), we need to handle redirect manually,
    # and because i need a working code tot test, i use 'requests' lib instead of urrlib (for the moment)

    r = requests.post(action, data=params, allow_redirects=False)

    # We send login form. If we get a 200, the login has failed, so error. We only need a redirect 302 here to continue
    if r.status_code != 302:
        raise CASLoginError('login failed')

    # Now, we get the redirect url: serviceurl/?ticket=ST.....
#    for cookie in r.cookies:
#        print(cookie)
   
#    # We check if the returned url hostname is the same the service url hostname
    return_url_parts = list(urlparse(r.headers['Location']))
    service_url_parts = list(urlparse(service))

    if return_url_parts[1] != service_url_parts[1]:
        raise CASLoginError('login failed')
